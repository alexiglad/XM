...
--model_name "jumpy" \
--model_size "vit_base" \
--run_name_hparams "img=image_dims,ps=patch_size,enc=backbone_type,xm_k=xm_best_of_k,jmps=jumpy_n_jumps,jt=jumpy_time_conditioning,jt_cont=jumpy_time_embed_cont,pred=jumpy_pred_type,inf_jmps=jumpy_inference_steps,prec=bf16m" \
--modality "VID" \
...
# we train in bf16-mixed here and not xdit_128.sh since we found jumpies to be more stable than dits (note this doesn't change jumpy's performance)
# also we didnt add time cond, can def add may improve perf (we just didnt try here)