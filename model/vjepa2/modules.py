...
class Block(nn.Module):
    ...
    def forward(self, x, mask=None, attn_mask=None, T=None, H_patches=None, W_patches=None, c=None):
        if self.use_adaln:
            shift_msa, scale_msa, gate_msa, shift_mlp, scale_mlp, gate_mlp = self.adaLN_modulation(c).chunk(6, dim=1)
            if isinstance(self.attn, RoPEAttention):
                y = self.attn(_modulate(self.norm1(x), shift_msa, scale_msa), mask=mask, attn_mask=attn_mask, T=T, H_patches=H_patches, W_patches=W_patches)
            else:
                y = self.attn(_modulate(self.norm1(x), shift_msa, scale_msa), mask=mask, attn_mask=attn_mask)
            x = x + self.drop_path(gate_msa.unsqueeze(1) * y)
            x = x + self.drop_path(gate_mlp.unsqueeze(1) * self.mlp(_modulate(self.norm2(x), shift_mlp, scale_mlp)))
        else:
            if isinstance(self.attn, RoPEAttention):
