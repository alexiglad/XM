...
def load_i3d_pretrained(device=torch.device('cpu')):
    ...
    if not os.path.exists(filepath):
        ...
        exit_code = os.system(f"wget {i3D_WEIGHTS_URL} -O {filepath}")
        if exit_code != 0 or os.path.getsize(filepath) == 0:
            os.remove(filepath) # dont leave a poisoned 0-byte file that makes every rerun fail at torch.jit.load
            raise RuntimeError(f"failed to download i3d weights, manually download {i3D_WEIGHTS_URL} to {filepath}")
